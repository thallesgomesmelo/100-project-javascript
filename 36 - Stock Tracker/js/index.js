...
function fetchTopStocks() {
  // Fetch data from api
  fetch("https://www.alphavantage.co/query?function=SECTOR&apikey=NB2OKEZBDFXCWS7P")
    .then(response => response.json())
    .then(data => {
      const stocks = data["Rank A: Real-Time Performance"];
      let html = "";

      // Loop throungh the stocks and generate html for each stock
      for (let i = 0; i < 10; i++) {
...
function fetchStockData(symbol) {
  ...
  fetch(
    `https://www.alphavantage.co/query?function=GLOBAL_QUOTE&symbol=${symbol}&apikey=NB2OKEZBDFXCWS7P`
  )
    .then(response => response.json())
    .then(data => {
